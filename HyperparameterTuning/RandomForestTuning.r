...
hyper_param_space <- ps(
  classif.ranger.max.depth    = p_int(15, 50),
  classif.ranger.num.trees    = p_int(500, 2000),
  classif.ranger.mtry         = p_int(1,16),
  classif.ranger.min.node.size   = p_int(1, 11)
)
...
# Save tuning results
saveRDS(tuning_instance, "randomForest_tuning_data.rds")
...
learner$param_set$values$importance = "impurity" # To get impurity-based feature importance
learner$train(task)

saveRDS(learner, "randomForest_final_model.rds")
